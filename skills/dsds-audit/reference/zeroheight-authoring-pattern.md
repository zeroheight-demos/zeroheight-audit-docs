...
This is the heart of the customer-facing audit. zeroheight authors cannot emit a
DSDS document (0.20.x's are YAML/JSON) — they write pages with titles, headings, tables, callouts, tabs, links,
...
- **DSDS intent:** a stable `id` tools/agents reference; distinct from display `name`.
...
- **DSDS intent:** each guideline item tagged `must` / `should` / `may` /
  `should-not` / `must-not` so a parser separates hard from soft.
...
### when-to-use guidance + alternative
- **DSDS intent:** a guideline with `framing: when-to-use` (recommend vs discourage
  via its `level`), plus an `alternatives` ref to the better option.
...
- **MCP sees:** stance from section structure; the internal link as the
  cross-reference.
- **Fidelity:** medium-high — stance is clear; the link approximates the
  `alternatives` ref.

### api (typed properties → `definitions`)
- **DSDS intent:** props as `definitions` items with id / type / default / required.
...
### variants / states (→ component `traits`)
- **DSDS intent:** `traits` — a boolean flag vs a named-option enum (first value the
  default), each with a stable `id`; `setBy` marks consumer-set vs component-set.
...
### anatomy / named parts (→ `definitions`, `context: anatomy`)
- **DSDS intent:** named parts/states each with an `id`, in a `definitions` section.
...
### theme (→ `colorScheme` + DTCG `source`)
- **DSDS intent:** a theme entry names its `colorScheme` and points a `source` at the
  DTCG file holding the overridden token values.
...
- **Fidelity:** high — the token-name column approximates which tokens the source overrides.

### testable guidelines (`checks` + `checkedBy`)
- **DSDS intent:** a guideline with a testable `statement`, a `checkedBy` mode
  (automated/assisted/manual), and a `checks` ref where automatable.
- **zeroheight move:** an **"Acceptance criteria" checklist/table** of testable
  ...
  button-name") and the mode. Best placed in the For Agents tab (below).
- **MCP sees:** a checklist of statements + tool names + mode.
- **Fidelity:** medium — readable and self-checkable by an agent; not runnable
  without a parser.

### for: agent sections (the agent layer)
- **DSDS intent:** firm, agent-only rules + look-alike disambiguation, in a section
  flagged `for: agent`.
...
first-class treatment later (elevate the section in `get-page`; map it to a
`for: agent` section with testable guidelines in a future export).
...
- **Acceptance criteria** are testable statements; give each a stable slug-style id
  and, where automatable, a `Check: tool / rule` and a `checkedBy` mode (automated /
  assisted / manual). This is exactly the shape a future export maps to DSDS testable
  guidelines (`checks` + `checkedBy`).
...
- **Guideline levels:** `Must` / `Should` / `May` / `Should not` / `Must not`
...
Some things only zeroheight can do: emitting a DSDS document (YAML/JSON),
server-deriving ids from slugs, structured `get-page` export, giving the For Agents tab first-class MCP
